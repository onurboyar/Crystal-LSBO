...
After creating this key, please copy it to line 85, and follow the intructions in README file.
"""
...
def target_property_wrapper(latent_space, run_id):
    ...
    with torch.no_grad():
        ...
        pred_formula, pred_abc, pred_ang, pred_latt, pred_site_coor, generated_elm_dict = convert_cif(ftcp,
                                        ...
                                        elm_str=joblib.load('data/element.pkl'),
                                        to_CIF=True,
                                        folder_name='{}/{}/CIF_run_id_{}_{}/'.format(exp_name,exp_name2, run_id, experiment_name),
                                        convertibility_only=False,
                                        print_error=False,
                                        )
        CIF = '{}/{}/CIF_run_id_{}_{}/{}.cif'.format(exp_name,exp_name2, run_id,experiment_name,random_counter)
...
for run_id in run_ids:
    ...
    for i in range(max_iter):
        ...
        res = torch.cat([res, torch.tensor([new_y]).to(device)])
    df = pd.DataFrame(res.detach().cpu().numpy())
    df.to_csv('{}/{}/labels_seed_{}.csv'.format(exp_name, exp_name2, run_id), index=False)
